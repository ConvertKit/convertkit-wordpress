...
class Read_Actions_Filters {
	 ...
	 * PHP files.
	 * 
	 * Results can be returned as an array, HTML or Markdown.
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @param 	mixed 	$prefix_required_replacement 	Optional prefix string replacement, to use if $prefix_required is found (e.g. $this->base->plugin->name --> wp_to_buffer_pro_)
	 ...
	 * @param 	string  $css_classes 					HTML Table CSS Classes (optional)
	 * @return	mixed									Output
	 */
	public function run( $folders, $extract_filters = true, $extract_actions = true, $return_format = 'html', $prefix_required = false, $prefix_required_replacement = false, $by_file = false, $css_classes = '' ) {
		...
		switch ( $return_format ) {
			...
			default:
				...
				if ( $extract_filters && count( $filters ) > 0 ) {
					$html .= $this->html( $filters, $by_file, 'filters', $css_classes );
				}
				if ( $extract_actions && count( $actions ) > 0 ) {
					$html .= $this->html( $actions, $by_file, 'actions', $css_classes );
				}
	 ...
	 * @param 	string 	$css_classes 	Table CSS Classes (optional)
	 * @return	string	HTML
	 */
	private function html( $results, $by_file, $type = 'actions', $css_classes = '' ) {
