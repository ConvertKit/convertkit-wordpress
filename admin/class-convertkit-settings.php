...
class ConvertKit_Settings {
	...
	public function enqueue_scripts( $hook ) {
	    ...
	    if ( 'settings_page__wp_convertkit_settings' === $hook ) {
            wp_enqueue_script( 'ck-admin-js', plugins_url( '../resources/backend/wp-convertkit.js', __FILE__ ), array( 'jquery' ), '', true );
	...
	public function get_tags() {
		...
		$tags = get_option( 'convertkit_tags', array() );
		$values = array();
		foreach ( $tags as $tag ) {
			$values[] = array(
				'value' => $tag['id'],
	...
	public function category_form_fields( $tag ) {
		...
		echo '<tr class="form-field ck-term-form-field"><th scope="row"><label for="description">ConvertKit Form</label></th><td>';
